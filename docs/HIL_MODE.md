...
| `pi-silence` **(EMS-driven, 2026-08-31)** | `hold-5050` at its 1.2 m/s default cruise, with the emulated Pi commander going **permanently silent at t = 8.0** (`pi_mute_after_s`) while the injection stream keeps running at full rate. Closes a verified coverage gap: `checkPiWatchdog()` (`.ino:4976-4985`) stamps its clock **only** in the 22-byte command branch (`:5043-5044`), independent of `hilLastFrameMs` (`:5132`), and every prior stimulus stopped both streams together. **`FAULT_PI_TIMEOUT` is REQUIRED.** ⚠️ 0x0010 is shared with `FAULT_HIL_LINK`, so the entry declares **`child_tx_healthy`**. From fw v25 that check READS the latched first cause off frame byte 16 (`ERR_PI_TIMEOUT` 0x05 vs `ERR_HIL_STALE` 0x10); on a fw v21–v24 board it falls back to the older inference (a continuous injection stream rules the alias out by elimination). Injection never stops, so no fw v23 run boundary forms and the latch persists; a mid-run warm reset here would prove contamination (and would clear `pi_ever_connected`, disarming the watchdog under test). |
...
- **The charger is simulated at the STATUS level only — its I2C transport is not.**
  ...
  **⚠️ DATASHEET CORRECTION (2026-08-31).** The Ag105's MPPT is an **input-voltage
  ...
  fw v24**: the firmware writes reg 0x02 and reports the count it believes is in
  force on observation-frame byte 15, and the plant applies that; the 18 V
  constant is only the fallback for a legacy frame, resistor mode, or the window
  before the first frame. ⚠️ **R1 is no longer a design contingency** — Table 7
