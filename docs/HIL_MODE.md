...
| `pi-silence` **(EMS-driven, 2026-08-31)** | `hold-5050` at its 1.2 m/s default cruise, with the emulated Pi commander going **permanently silent at t = 8.0** (`pi_mute_after_s`) while the injection stream keeps running at full rate. Closes a verified coverage gap: `checkPiWatchdog()` (`.ino:4976-4985`) stamps its clock **only** in the 22-byte command branch (`:5043-5044`), independent of `hilLastFrameMs` (`:5132`), and every prior stimulus stopped both streams together. **`FAULT_PI_TIMEOUT` is REQUIRED.** ⚠️ 0x0010 is shared with `FAULT_HIL_LINK` and the observation frame carries no `error_code`, so the entry declares **`child_tx_healthy`** — a continuous injection stream rules the alias out by elimination. Injection never stops, so no fw v23 run boundary forms and the latch persists; a mid-run warm reset here would prove contamination (and would clear `pi_ever_connected`, disarming the watchdog under test). |
