...
| `mppt-tracking` **(EMS-driven, 2026-08-31)** | the `charge-regen` cruise/brake profile driven by the `mppt-harvest` strategy, which asserts `charge_goal` on the braking windows (regen path) **and** on the 0.4 m/s low-cruise plateaus (FC path) — with the Ag105's **MPPT input-voltage threshold** emulated (`mppt_emulation`, 18 V default, `AG105_Silvertel.pdf` p.10). The FC path feeds the charger from the ~15.95 V bus, which cannot clear the threshold, so this is the first scenario in which `MPPT_DISABLE` is **causally load-bearing** rather than a flag-only control. ⚠️ **Asserts a model PREDICTION, contingent on open question R1** (MPPTS resistor unconfirmed): the firmware releases tracking only once the charger reports ready, and releasing it is what stops the charging that made it ready, so the two are predicted to **hunt** at ~50 % duty — period **~40.05 ms** (MEASURED median, campaign `20260831_191509`; the offline probe's 80.0 ms figure was a mis-derivation, and the 138 toggles that campaign counted arithmetically require the shorter period), with `I_charge` equilibrating near half the ceiling. The suite's toggle ceiling is **2200 ticks** — derived from the 3.0 s in which the pin *can* be HIGH (3 × 1.5 s of inset cruise-charge window, less 3 × `AG105_SETTLE_S`), which separates the ~1500-tick hunt from the ~3000-tick stuck-high outcome; the earlier 10000 could not bind and the check was inert. A campaign that does **not** see the hunt is evidence about R1 — a hardware finding, not a scenario defect. `chg_i_ceiling_a` **1.0 A** (the FC charge path is single-source: 0.15 aux + ~0.06 motor + 1.0 = 1.21 A, 14 % under `LIMIT_I_FC_MAX`). |
| `charge-to-full` **(2026-08-31)** | 130 s **standstill** FC-path charge from `--soc0 0.990` (the suite overrides it, mirroring `soc-depletion`): the first run in this suite to reach Ag105 GENSTAT **011 Fully Charged** with the CV flag and the current taper. 0.005 of a 5 Ah pack = 90 A·s = 90 s at the 1.0 A ceiling, so FULL is expected ~t = 100. Also pins the firmware's deliberate **no-action** response — `ag105IsReady()` accepts FULL so MPPT stays released, `chargingControl()` never reads GENSTAT so `FC_CHARGE_ENABLE` stays open, and FULL is not an error GENSTAT. ⚠️ `mppt_emulation` is deliberately **OFF** here: the 18 V gate would block this very path. ⚠️ No drive-channel coverage — `v_setpoint` is 0 throughout, which is what makes the single-source budget (1.15 A) work. `CHARGER_STAT` (pin 6) is on **neither** HIL frame, so its Fully-Charged blink signature is out of scope. |
...
- **The charger is simulated at the STATUS level only — its I2C transport is not.**
  ...
  **⚠️ DATASHEET CORRECTION (2026-08-31).** The Ag105's MPPT is an **input-voltage
  ...
  is the only scenario that declares it. ⚠️ Whether this board fits an MPPTS
  resistor is **open question R1** — an off-board MPPTSEL header exists on the
  schematic, contents unconfirmed. The threshold constant and that scenario's
  expectations move together if R1 resolves to a fitted resistor.
