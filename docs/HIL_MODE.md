...
| 15 | 1 | `mppt_thresh_count` | **APPENDED fw v24.** The Ag105 reg-0x02 count the firmware *believes* is in force (`ag105MpptRegCnt`). ⚠️ Under `HIL_SIM` this value is a **fiat mirror** recomputed from injected `V_chg`, not the fw v24 threshold manager's output — see the mirror boundary below. `0xFF` = external-resistor mode / never written (`AG105_MPPT_N_RESISTOR`, the boot value); `0..250` map to `11.0 + 0.088·N` volts. Clamp band `[15, 27]` = 12.320–13.376 V |
...
| `v-bus-sense-offset` **(hifi only)** | two sensed-`V_bus` excursions below `LIMIT_V_BUS_MIN` — 12 ms at t = 5.0 (must **not** latch) then 60 ms at t = 8.0 (must latch): `UV_BUS_DWELL_LATCH_MS` 20 ms from both sides. The 3 s gap clears the leak. hifi because its offset is sense-path-only and so perturbs nothing but the quantity under test |
...
| `ems-y-b30-v1` / `ems-y-b30-v3` **(EMS-driven, 2026-08-31)** | the firmware's OWN `'Y'` combined drive-cycle + power-share table (16 regions, 40 s, the firmware's `COMBINED_PROFILE` region table) commanded from the EMS layer instead of the USB console, at Vmax **1** and **3** m/s with the firmware's documented share bound **b = 0.30**. Carries a **+0.60 A** `aux_preload_a` so the source total (0.75–2.01 A) stays above the 0.60 A closed-loop governor gate for the whole table — the objective is **closed-loop share tracking**, and the bound keeps the share inside `[DROOP_R_MIN, DROOP_R_MAX]` so no cut occurs. |
| `ems-y-b00-v1` / `ems-y-b00-v3` **(EMS-driven, 2026-08-31)** | the same table with **b = 0.00**, so regions 6 and 11 command share **1.00** and **0.00** — outside `[DROOP_R_MIN 0.15, DROOP_R_MAX 0.85]` — and `updateShareSetpointCutoff()` opens `BT_BUS` and then `FC_BUS`. The objective is the **cut-and-RESTORE topology**, including the first assertions in this suite that a latch is ever *released*. **No preload:** the cut's own `SHARE_CUT_MAX_HANDOFF_A` 0.5 A/channel guard would refuse the latch above that current. ⚠️ Consequence: at Vmax 1 the source total never reaches the governor gate, so that run's share loop is **open-loop feedforward** — do not read share-tracking numbers off a `b00` run. |
| `ems-ftp75-5050` / `ems-ftp75-socband` **(EMS-driven, opt-in, 2026-08-31)** | the **EPA FTP-75** cycle at raw **t = 0..340 s inclusive** (341 samples at 1 Hz) — the segment of `references/Systemic_Scaling_of_Powertrain_Models_with_Youla_Driver_Control.pdf` — scaled by one constant (3.0/56.7 m/s per mph, so the 56.7 mph peak lands on 3.0 m/s) and driven by `hold-5050` and `soc-band` respectively. Profile is GENERATED: `tools/gen_ftp75_profile.py` -> `tools/ftp75_profile.py`, from the sha256-verified `references/drive_cycles/ftpcol.txt`. 350 s each, `aux_preload_a` **+0.65 A**. `run_hil_suite.py` renders both **SKIPPED** unless `--with-ftp75` is given — a run-time gate (~11.7 min for the pair), not a coverage one. ⚠️ The preload puts the source-total floor at 0.800 A, above `soc-band`'s 0.60 A charge-admission threshold, so the **charge branch is out of reach on `ems-ftp75-socband` by construction**; `ems-soc-band` remains the home of the charge-window assertion. |
| `mppt-tracking` **(EMS-driven, 2026-08-31; objective INVERTED at fw v24)** | the `charge-regen` cruise/brake profile driven by the `mppt-harvest` strategy, which asserts `charge_goal` on the braking windows (regen path) **and** on the 0.4 m/s low-cruise plateaus (FC path) — with the Ag105's **MPPT input-voltage threshold** emulated (`mppt_emulation`) at whatever count the BOARD reports on observation-frame byte 15; the 18 V datasheet default applies only until the firmware writes reg 0x02. This is the first scenario in which `MPPT_DISABLE` is **causally load-bearing** rather than a flag-only control. ⚠️ **THE OBJECTIVE INVERTED AT fw v24.** Under fw v23 the module sat at its 18 V default, the FC path could not clear it, and the firmware and the module **hunted** — 138 `MPPT_DISABLE` toggles at a **~40.05 ms** median period, measured on hardware in campaign `20260831_191509`. fw v24's threshold manager writes reg 0x02 to (windowed-min `V_chg` − 3.0 V), clamped in counts to **[15, 27] = 12.320–13.376 V**, so the module stops refusing and harvest holds. **The hunt is now the FAILURE signature**: the retired 2200-tick ceiling is replaced by a phase-free **edge census** (`mppt_no_hunt`, 3–8 rises across the cruise windows — one per window; the fw v23 hunt is ~69), the Low-Power refusal check is inverted to an absence bound, and the new `mppt_thresh_cnt` CSV column is a mirror-only byte exercise: under HIL the real manager is bypassed, and the column evidences the fiat mirror's formula, not the manager's execution. R1 is no longer a contingency: Table 7 makes reg 0x02 0–250 *register* mode and ≥251 the resistor, so a firmware write overrides any fitted resistor. `chg_i_ceiling_a` **1.0 A** (the FC charge path is single-source: 0.15 aux + ~0.06 motor + 1.0 = 1.21 A, 14 % under `LIMIT_I_FC_MAX`). |
| `charge-to-full` **(2026-08-31)** | 130 s **standstill** FC-path charge from `--soc0 0.990` (the suite overrides it, mirroring `soc-depletion`): the first run in this suite to reach Ag105 GENSTAT **011 Fully Charged** with the CV flag and the current taper. 0.005 of a 5 Ah pack = 90 A·s = 90 s at the 1.0 A ceiling, so FULL is expected ~t = 100. Also pins the firmware's deliberate **no-action** response — `ag105IsReady()` accepts FULL so MPPT stays released, `chargingControl()` never reads GENSTAT so `FC_CHARGE_ENABLE` stays open, and FULL is not an error GENSTAT. ⚠️ `mppt_emulation` is deliberately **OFF** here, and stays off under fw v24: the 18 V gate would have blocked this very path outright, and while the fw v24 clamped threshold no longer would, this run is a continuously-fed standstill charge and the gate would simply be inert — the two scenarios test different things. ⚠️ No drive-channel coverage — `v_setpoint` is 0 throughout, which is what makes the single-source budget (1.15 A) work. `CHARGER_STAT` (pin 6) is on **neither** HIL frame, so its Fully-Charged blink signature is out of scope. |
| `pi-silence` **(EMS-driven, 2026-08-31)** | `hold-5050` at its 1.2 m/s default cruise, with the emulated Pi commander going **permanently silent at t = 8.0** (`pi_mute_after_s`) while the injection stream keeps running at full rate. Closes a verified coverage gap: `checkPiWatchdog()` stamps its clock **only** in the 22-byte command branch (`:5043-5044`), independent of `hilLastFrameMs` (`:5132`), and every prior stimulus stopped both streams together. **`FAULT_PI_TIMEOUT` is REQUIRED.** ⚠️ 0x0010 is shared with `FAULT_HIL_LINK`, so the entry declares **`child_tx_healthy`**. From fw v25 that check READS the latched first cause off frame byte 16 (`ERR_PI_TIMEOUT` 0x05 vs `ERR_HIL_STALE` 0x10); on a fw v21–v24 board it falls back to the older inference (a continuous injection stream rules the alias out by elimination). Injection never stops, so no fw v23 run boundary forms and the latch persists; a mid-run warm reset here would prove contamination (and would clear `pi_ever_connected`, disarming the watchdog under test). |
| `share-staircase` **(2026-08-31)** | 47 s **motor-free** (`v_setpoint` 0 throughout) two-phase share sweep. **Phase A** at I_tot ~1.2 A: a 0.80 → 0.20 staircase in 0.10 steps every 3 s, straddling the governor's rails `SHARE_MINORITY_I_MIN_A/I_tot` = **[0.25, 0.75]** — the clip band campaign TP0170–0180 measured incidentally becomes a designed observable, swept both ways. **Phase B** after a load **drop to ~0.55 A** at t = 29: excursions to 0.95 and 0.05, outside `[DROOP_R_MIN, DROOP_R_MAX]`, cutting and then **restoring** `BT_BUS` and `FC_BUS`. The two loads are forced apart: at 1.2 A a 50/50 split is 0.60 A, over the cut's `SHARE_CUT_MAX_HANDOFF_A` 0.5 A guard, so the latch would *defer*. Four **latency measurements** (`switch_fall_latency_ms`, both edges) are the deliverable. ⚠️ Corrected premise: the observed [0, 20) ms spread is **command-arrival phase** (`PI_CMD_HZ` 50), not a firmware tick — `POWER_BAL_PERIOD_US` and `SHARE_CTRL_TS_US` are both 1000 µs. |
| `fw26-clamp-cruise` **(fw v26, 2026-09-02)** | 38 s **motor-free** (`v_setpoint` 0 throughout) two-source high-total run, and the ONLY stimulus that reaches the fw v26 source current-ceiling clamp. An `aux_preload_a` of **1.85 A** puts the two-source total at a constant **2.00 A** — clear of the clamp's **1.55 A** reachability threshold (`SHARE_GOV_I_FC_CEIL_A` 1.25 A plus the 0.30 A minority floor), which only `ems-y-b30-v3` also reaches, and only as an ~11 ms transient at t ~ 27.02 s (reconstructed commanded `I_fc` 1.5180 A; see `tools/probes/probe_fw26_clamp_reachability.py`). The next-highest commanded fuel-cell current on the registered set is `ems-sdp`'s 1.1861 A. **Phase A** (t = 8..24, share **0.75**): the unclamped fuel-cell demand is 1.50 A and the clamp must hold it at **1.25 A**, forcing the remaining 0.75 A onto the battery; aux-byte **bit 4** is the evidence. **Phase B** (t = 26..34, share **0.40**): the demand falls to 0.80 A, the clamp releases, and the same run supplies its own negative control. Motor-free for `share-staircase`'s reason: the acceptance band on `I_fc` is 0.10 A wide and a drive transient would move `I_tot` and with it both the engagement boundary and the band. ⚠️ Every bound is a `tools/governor_model.py` **walk**, not a measurement — see `docs/HIL_SCENARIOS.md`. |
| `fw26-clamp-sweep` **(fw v26, 2026-09-02)** | 84 s `'Y'`-shaped leg: twelve 6 s regions of velocity setpoint against commanded share, scripted through the Pi command path, crossing the clamp boundary on **both axes**. Eight of the twelve regions command a share of 0.84, and the two-source total is stepped between **1.20 A** and **2.02 A** across them (an `aux_preload_a` of **1.05 A** sets the 1.20 A standstill floor and the motor carries it over the 1.55 A threshold); two more step the share between 0.84 and 0.40/0.20 at a fixed total, so a release BY SHARE and a release BY TOTAL are distinguishable. Five clamping regions and seven inert ones. **The droop-code pins:** the three standstill sub-threshold regions have their codes pinned to the walk at +/-2 % of the 12-bit gain CODE (the raw word carries the 0x1000 command nibble, which dilutes a band applied to the word). That arm is MODEL FIDELITY, not clamp evidence -- the clamp is the identity there by construction. The fw v25/v26 discriminator is a CLAMPED region's pair: region 2 reads (5088, 5679) with the clamp and (4824, 7837) without it, pinned at +/-10 % of the code. No commanded share touches the band edge (0.84, not 0.85 — a float32 round-trip past `DROOP_R_MAX` would be a cut). ⚠️ Region totals are the host demand model's; the board's drive loop sets the real ones. |
...
- **Idle → Run zeroes `v_setpoint` and resets the drive controller**
  (`doState1()`). The real setpoint therefore arrives on the
...
- **Once in Run the 50 Hz stream is load-bearing.** The Pi watchdog
  (`PI_TIMEOUT_MS` = 500) arms after the first command packet and
...
`TP0178` (2026-08-17b) recorded `V_bus` sagging to **12.1489 V** — 0.1489 V **above**
`LIMIT_V_BUS_MIN`, so the firmware's leaky dwell integrator accumulates **0.0 ms** and
it correctly did *not* fault. (⚠️ Record corrected 2026-08-31: this example used to
say "only ~10 ms long, under the 20 ms dwell". That did not survive replay — the floor
never crosses the limit, so there is no dwell to be under, and the sub-12.15 V
excursion is 1–3 ms wide. The margin is a **voltage** margin.) Replaying it verifies
that judgement stays correct in the current build:
...
Expected: the injected `V_bus` column in `hil_TP0178.csv` reproduces the 12.1489 V
trough (it is the recorded one), the observation frame's `mainState` never reaches
99, and `fault_flags` stays 0 throughout — the sag never reaches the limit. Then push it
...
- **The charger is simulated at the STATUS level only — its I2C transport is not.**
  ...
  → "Charging" with a first-order ramp toward the configured 2.5 A ceiling. CV taper
  exists only as the SoC-triggered Fully-Charged branch (`charge-to-full` is the
  scenario that reaches it).
  **⚠️ DATASHEET CORRECTION (2026-08-31).** The Ag105's MPPT is an **input-voltage
  threshold** regulator, **not** a perturb-and-observe tracker: charging commences
  only above a threshold settable 11–33 V by an MPPTS resistor or I2C register
  `0x02`, and **defaulting to 18 V with MPPTS open** (`AG105_Silvertel.pdf` p.10).
  The "perturb-and-observe" wording in `teensy_controller.ino`'s comments and in
  CLAUDE.md §3 is repo lore with no datasheet backing. Nothing in the *firmware*
  depends on the distinction — it drives one GPIO either way — but the plant model
  does. By default `MPPT_DISABLE` still only clears the two tracking flags in the
  status byte; a scenario declaring **`mppt_emulation`** gets the threshold gate
  instead (Layer 1: the threshold, **not** the tracking dynamics). `mppt-tracking`
  is the only scenario that declares it. **The threshold VALUE is dynamic from
  fw v24**: the count carried on observation-frame byte 15 is what the plant
  applies; the 18 V constant is only the fallback for a legacy frame, resistor
  mode, or the window before the first frame.
  ⚠️ **BUT UNDER `HIL_SIM` BYTE 15 IS A FIAT MIRROR, NOT THE MANAGER'S OUTPUT**
  (corrected 2026-09-02, review PLANT-R1-F1). The fw v24 threshold manager is
  **never called** in the HIL branch; the branch recomputes `ag105MpptRegCnt`
  from the injected `V_chg` on every settled tick, gated only on
  `chargerHasPower()`. That bypasses the window, the ≤2-per-session ratchet, the
  3-count deadband, the ≤8-per-boot EPROM budget, the read-verify handshake —
  and the **regen exclusion** (the manager samples only while
  `fcChargePathIsPowering()`, i.e. FC_CHARGE high AND REGEN low). Measured on
  campaign `hil_report_20260902_011926`: the mirror reads 27 inside a braking
  window at `V_chg` 18.08 V where the board would hold 15–19, and 11.8 % of a
  run's ticks differ by up to 12 counts. An HIL run therefore evidences the count
  arithmetic, the clamp band and the frame plumbing — never that the manager ran
  or that a write policy was honoured. ⚠️ **R1 is no longer a design contingency** — Table 7
  encodes reg 0x02 values 0–250 as *register* mode and ≥251 as the external
  resistor, so a firmware write overrides any fitted MPPTS resistor. Whether the
  board fits one now only decides the threshold *before* the first write.
...
- ~~**Regen is floored at zero bus current.**~~ **SUPERSEDED 2026-09-01 (WP-C).**
  Braking energy is now modelled end to end — flywheel → VESC (regen-side current
  clipped at `VESC_REGEN_I_MAX_A`) → V-MOT → chopper clamp / Ag105 → pack SoC.
  Regen still never returns energy to VBUS, but for the physical reason rather
  than by fiat: `MOT_PWR` is an ideal diode and blocks the back-feed. See
  `docs/HIL_PLANT.md` §3.4 for the model, its three `TODO(verify)` constants, and
  the baseline-era warning on pre-WP-C regen traces.
