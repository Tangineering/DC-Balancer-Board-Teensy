...
| `scp-inrush` **(hi-fi only)** | VESC input capacitance forced to the **top of the envelope (0.9 mF)**, and a **5.0 A draw on the V-MOT node** (behind the switch, *not* `i_aux` on VBUS) declared **from t = 0** — `Plant.step()` applies it only while `MOT_PWR` is closed, so it appears exactly at the bring-up P3 close and the ramp runs into it while still in `SOFT`. The old +6 A at t = 8 s arrived when `MOT_PWR` had been ON since t ≈ 0.62 s, and the foldback branch exists only in `SOFT`: **zero** `scp_cut`/fold events fired and the run was a plain bus overload. | RT1987 soft-start **foldback** on `MOT_PWR` | `scp_cut` + `sw_ring` entries in the event sidecar. Verified offline: the margin holds at 2 A (soft-start completes, `V_mot` reaches 15.1 V) and breaks at ≥ 4 A into a **64 ms burst-retry cycle** — the Death-5-class ring pattern. **Not** the Death-5 stimulus itself: that was a full-bus hot-plug onto a discharged node, no longer reproducible (`MOT_PWR` carries a 100 nF CSS and the firmware pre-charges the node). This is the nearest *legitimate* case that can still bind the foldback. Ring peaks here stay under the 20 V abs-max because the cut happens at low `V_mot`; a cut at full bus on `--trace-config long` (BT, 3.480 nH) does cross it. |
...
`scp-inrush` load (6 A at the time of that fix round; the scenario now declares 5.0 A
from t = 0) still folds at ~5.6 A and cuts with the 64 ms retry, and completes
