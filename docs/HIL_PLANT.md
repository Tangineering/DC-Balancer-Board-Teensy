...
(`AG105_MPPT_V_THRESH` 18.0 V, `AG105_MPPT_V_HYST` 0.5 V — the hysteresis is on the
voltage *comparison* only, never on the pin, which is the firmware's output and must
not be filtered). Semantics, and the asymmetry is the datasheet's own: the threshold
...
**⚠️ Open question R1:** whether this board fits an MPPTS resistor is
**unconfirmed** — an off-board MPPTSEL header exists on the schematic, contents
unknown. With the header open the threshold is the 18 V default modelled here. If a
resistor sets a lower one, `AG105_MPPT_V_THRESH` and `mppt-tracking`'s expectations
move **together**, and a campaign that fails to reproduce the predicted hunt is
evidence about R1 rather than a scenario defect.
...
| **Charger status-level only; MPPT modelled at the THRESHOLD, not the tracking.** `I_charge` and `ag105_status` are injected (§4.6), so `chargingControl()`'s readiness gating and the GENSTAT fault check are live and testable. SoC and the CV/Fully-Charged branch are modelled (§4.2). **MPPT (2026-08-31, scoped):** with `mppt_emulation` the part's real mechanism — the **input-voltage threshold** (`AG105_MPPT_V_THRESH`, datasheet p.10; **not** perturb-and-observe) — IS modelled, and `MPPT_DISABLE` becomes causal. The **tracking dynamics** (how the module walks its operating point once above the threshold) are **not** modelled, and neither is the I2C transport or config handshake. Off by default. | Sequencing and status-decode logic around the charger are meaningful HIL results, and so is the *threshold gate*'s interaction with the firmware's readiness-gated MPPT release (`mppt-tracking`). Charger **tuning** and **harvest-efficiency** results are still not available. ⚠️ The `mppt-tracking` hunt is a **prediction contingent on R1** (MPPTS resistor unconfirmed), not a validated behaviour. |
...
| `ems-sdp` **(EMS-driven, CAUSAL SDP POLICY, 2026-08-31)** | the SAME 61 s profile and the SAME 1.0 A drain as `ems-soc-band` and `ems-dp-replay` — the entry is *derived* from the first and shares its `ems_v_profile` LIST OBJECT, and `apply_scenario()` matches all three names on one drain branch, so the three-way comparison is on a bit-identical stimulus. Charge ceiling **0.8 A**, inherited. `electrical: any` (unlike `ems-dp-replay`, this leg has no offline hydrogen accounting that must match the engine). | The **`sdp-v1`** strategy: a STATE-indexed policy (SoC x demand bin) baked offline by stochastic DP (`tools/sdp_ems_solver.py`, artifact `tools/sdp_policies/sdp_policy_v1.json`), looked up every `decision_dt_s` = 1 s and held between decisions. It is the CAUSAL optimal-by-construction leg between the `soc-band` heuristic and the non-causal `dp-replay` bound. | Measured offline against the shipped artifact: the strategy captures `soc0` and regulates around it (`target + (soc - soc0)`), the demand normalizes into the artifact's -1.125..+1.640 W span and CLAMPS to the top bin at bench power levels, and the policy there is bang-bang about its target — so the TABLE asks for share **1.0000** and `charge_goal` is **0** for the whole run (the solver forbids charging in demand bins 12-24; that zero-charge property is ROBUST, not incidental — a reviewer perturbation sweep, 2026-08-31, re-solved 600 charge cells under +/-1 %/+/-5 % alpha, 0.5-0.9 A ceilings, 12-16.5 V bus and +/-1 %/-20 % capacity and the decision flipped only at a 1.2 A ceiling, where the FC-budget rule forbids charging anyway). Precisely: share 1.00 for SoC nodes in (0.550, 0.600], 0.00 above AND at the exact grid-floor node 0.550 (a solver-side clamp-tie degeneracy, its D3/D8) — the floor node is unreachable in this scenario (it needs SoC to fall 0.05 vs the run's ~0.006) but a longer or heavier-drain reuse of `sdp-v1` could reach it and would command the most-discharging split there; the clamp emits that as 0.15, in band, so no OC reasoning changes. The emitted share is the **HARDWARE-ENVELOPE CLAMP 0.8500** (`[SOC_BAND_SHARE_MIN, SOC_BAND_SHARE_MAX]`, `SdpStrategy.clamp_share()` — the same clamp `soc-band` applies, actuation-side only, table untouched, raw value counted and printed): a rail would sit outside `[DROOP_R_MIN, DROOP_R_MAX]`, cut BT off the bus and run single-source FC into the ~1.45 A drain past `LIMIT_I_FC_MAX` — an OC latch that would truncate the run and destroy the three-way comparison. At the clamp the firmware's own governor clips further to `1 - I_min/I_tot` = 0.795 at the 1.462 A drain peak: **I_fc ~1.16 A (17 % margin)**, BT minority exactly `SHARE_MINORITY_I_MIN_A` 0.30 A, no cut attempted, so `SHARE_CUT_MAX_HANDOFF_A` never enters. **Expected fault-free over the full 61 s**; `h2_cum_g` ~1e-2 g. ⚠️ Read its `delta_soc` with the hydrogen number — a sustained 0.85 FC-heavy split spends less pack charge than the other two legs by construction. ⚠️ SIM-ONLY (plant-truth SoC), like `soc-band`. PROVENANCE: the artifact's POLICY-BLOCK sha256 is `dbe42d1b…` (recipe: `sha256(json.dumps(doc["policy"], sort_keys=True))`) — the decision law, stable across a regeneration that did not change it, and the digest to quote. The artifact's FILE sha moves on every regeneration (`generated_utc`), so it is recorded PER RUN in the CSV meta sidecar under `config.sdp_policy` alongside the policy-block sha, `generated_utc`, the grid shape and the TPM sha. |
