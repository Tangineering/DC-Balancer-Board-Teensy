...
#### Standing rule: every EMS strategy gets the full command band

⚠️ **OPERATOR RULE, 2026-09-02.** Every EMS strategy has access to the full
firmware command band **[0.15, 0.85]**, and the benchmarks are strategies for
this purpose. A bound solved over a control set narrower than the one the
causal policies use is not a bound on them: before this rule the DP grid was
[0.25, 0.75] while `SdpStrategy.clamp_share()` clamped to the band, so on every
`ems-*-sdp` leg the causal run BEAT its own lower bound.

The band is taken from `governor_model.GOV_CONST["DROOP_R_MIN"/"DROOP_R_MAX"]`
and never re-typed. Commanding the rails is safe because
`updateShareSetpointCutoff()` compares **strictly**, so 0.15 and 0.85 are IN
band, and the firmware carries `SHARE_CUTOFF_HYST` 0.01 beyond them on top of
that. `sdp-v4` has railed at 0.8500 on 100 % of ticks across campaigns
`20260902_011926` and `20260902_041414` with zero hazard cuts.

**Single-source commands (share 0 and 1) are MPC-ONLY** (operator ruling,
2026-09-02). The MPC is to gain them as candidates; the **DP and the SDP are
not**. A single-source command is issued through the **setpoint latch** rather
than through the share loop, and it is subject to the fw v25 **share-cut load
...
### 6.2 The compressed-cycle legs (`ftp75c`, 2026-09-02, gated behind `--with-ftp75c`)

Five scenarios drive a **time-compressed FTP-75 cycle on a road-load-compensated
plant**. They are the first drive-cycle family on this rig that regenerates at
all, and everything specific to them follows from that one fact. Read
`docs/HIL_PLANT.md` §3.5 and §3.6 before reading a trace from any of them; the
design record is `docs/modeling/ftp75c_regen_cycle_design_20260902.md`.

**The stimulus.** `tools/ftp75c_profile.py` is the same EPA source segment as
`ftp75` (raw t = 0..340 s, sha256-verified) with the time axis multiplied by
0.5 before the profile offset, generated by
`tools/gen_ftp75_profile.py --time-factor 0.5` and never hand-edited. The
velocity axis is untouched, so the table keeps the same **234 points** and the
same 3.0 m/s peak, now at t = 125.0 s, and **every acceleration doubles** to a
peak of ±0.3492 m/s². The table runs t = 5.0..175.0 s; `FTP75C_RUN_EXIT_S` is
176.0 s and `FTP75C_DURATION_S` is 180.0 s, on the FTP-75 family's derivation
term for term. All five legs share one profile list object, one
`aux_preload_a` of 0.0 A and one `chg_i_ceiling_a`, which is what the frontier's
stimulus-coherence precondition asserts.

**The plant.** Every leg declares `"drag": "scaled-air"`, so the measured rig
road load is replaced by the study vehicle's scaled air drag, `k_air` =
0.0598 N/(m/s)², `F_c` = 0. That cuts the tractive demand by roughly 4.5× and is
what creates the regenerative braking energy: 51.25 % of braking kinetic energy
reaches the shaft, 15.662 J survives the 1.5 A VESC clip and 12.530 J reaches
the V-MOT node. ⚠️ **The compensated plant is HIL-only** and needs a second
road-load motor to replicate on the bench (`HIL_PLANT.md` §3.5). `drag` is a
fingerprinted stimulus key, so a leg run under `--drag rig` is a different
vehicle and must not be compared with one run as declared.

**The regen manager.** `charge_goal` is forced to 1.0 over **six derived
windows carrying 19.600 s**, 11.5 % of the 170 s cycle, by a **common layer over
every strategy** rather than by per-strategy logic. Every strategy brakes at the
same instants because every strategy follows the same profile, so making the
layer common is what keeps a comparison between these legs a comparison of share
policies. The windows carry a 0.20 s lead-in and a 0.20 s lead-out, and the
lead-in is safety-critical: asserting `charge_goal` one tick before the
commanded current has gone negative takes `chargingControl()`'s CRUISE branch,
calls `assertFcChargeEnable(true)`, drops BT off the bus and creates the
single-source condition that has latched `OC_FC` before. Windows are re-derived
at run time from the **resolved** drag mode, so a `--drag rig` control run of any
of these scenarios gets the empty window list its own physics implies.

**Observables, identical on all five legs.** The regen path is
strategy-independent by construction, so one builder emits the same four checks
...
- `ftp75c_regen_charge`: `I_charge` ≥ 0.06 A inside the cycle's first long
  window (62.0, 68.0) s, against a modelled peak of 0.124 A. A closed switch
  with no current behind it satisfies the duty check and nothing else.
...
The four legs built by the family builder (all but `ems-ftp75c-mpc`, which is
built by `_mpc_expectation()` and carries the MPC list instead) additionally
carry `ftp75c_cadence` (≥ 140 000 rows in the Run
window, so every window-scoped bound is judged on a complete cycle),
`ftp75c_peak_commanded` (`cmd_v_sp` ≥ 2.85 over (120, 130) s) and
`ftp75c_fc_bounded`, an `I_fc` **ceiling** at twice that leg's own modelled peak.
...
here: the credit is **1.1729 C**, about 1.4 % of the cycle drain, a SoC gain near
+5.5e-5 against a −0.0054 excursion. It is invisible in the SoC trace. Read the
harvest off `I_charge`, the `chopper_clamp` event's `energy_j` and the plant's
`regen_energy_j` counter.

⚠️ **The credit is identical on all five legs, 1.1729 C on the same six
windows**, because the manager is common and the credit is share-independent.
`ftp75c` validates the regen model end to end and REDUCES the DP's regen
divergence rather than closing it: the bound now earns the same credit the run
does, and what remains is the Ag105 settle and ramp, which cost roughly the
first 0.9 s of every window and hold the realizable fraction near 70.7 %. However, it is **not** expected to reorder the strategies, and a
reordering on this stimulus is a defect signal rather than a result.

⚠️ **Every band on this family is PROVISIONAL.** No campaign has run any of it.
The values are governor-walk predictions, and `ETA_REGEN` = 0.80 and
`VESC_REGEN_I_MAX_A` = 1.5 A are both `TODO(verify)`, with the whole harvest
column linear in the first.

**All five expect `allow_only: 0`.** The peak source total is 0.3311 A against
`LIMIT_I_FC_MAX` 1.4 A, so the family has far more margin than any rig-drag leg.
The specific latch to watch is `FAULT_SWITCH_CONFLICT` (0x0008): it would mean
the regen manager provoked the CRUISE branch inside a braking window, and the
window derivation is then the first thing to re-derive.

#### ems-ftp75c-5050 (180 s, any engine, EMS `hold-5050`)

- **Tests:** the constant 50/50 split, so any share deviation belongs to the
  firmware's share loop and the plant and never to the EMS. On this family it is
  also the cleanest read of the regen path, because `hold-5050` commands no
  `charge_goal` of its own at all; every assertion of it comes from the common
  manager. It is the one leg on which `chg_i_ceiling_a` is declared although the
  strategy never commands charging, because here the manager does.
- **Pass/fail:** the family's checks, with `I_fc` ≤ 0.3536 A (twice the walk
  peak of 0.1768 A, 25 % of `LIMIT_I_FC_MAX`). Walk: h2 0.006288839 g,
  ΔSoC −0.001926, 1.1729 C to the pack.

#### ems-ftp75c-socband (180 s, any engine, EMS `soc-band`, FRONTIER REFERENCE)

- **Tests:** the causal charge-sustaining policy, and the reference leg of both
  compressed-cycle frontier tuples.
- ⚠️ **It runs PER-SCENARIO charge thresholds, and the override is what makes
  it usable as a reference.** `SOC_BAND_CHARGE_ENTER_ITOT_A` is 0.60 A and this
  cycle's entire source total peaks at 0.3311 A, so the shipped threshold sits
  above every instant of the cycle: the leg would admit a charge window at the
  first cruise sample and never exit it by current, which would make it a
  charge-saturated control rather than a policy. Both thresholds are therefore
  divided by `DRAG_INERTIA_RESIDUAL` 4.486628331803267, the same factor the
  distribution POSITION it occupies, giving **0.18074 A enter and 0.33107 A exit**. The override is
  a scenario key, so the 61 s and `ftp75` legs are untouched.
- **Pass/fail:** the family's checks, with `I_fc` ≤ 0.3712 A (walk peak
  0.1856 A); plus `ftp75c_socband_not_saturated`, `SW_FC_CHARGE` set for at most
  120 000 ticks of the 170 s cycle. A FAIL there means the override did not
...
- ⚠️ **No new SDP artifact was solved for this stimulus, deliberately.** The
  regen credit enters through the plant and the pack, not through the policy's
  decision law, and `sdp_policy_v4`'s axes, relative SoC and a demand bin, are
  stimulus-independent by construction. Re-solving would have produced a second
  artifact differing from `sdp_policy_v4` only in the demand map it was fitted
  on, with no campaign able to tell the two apart.
- **Pass/fail:** the family's checks, with `I_fc` ≤ 0.5744 A (walk peak
  0.2872 A, the highest of the four). Walk: h2 0.009897751 g, ΔSoC −0.000464, and
  the policy holds its 0.85 share rail through the cycle, which is why it burns
  more hydrogen and drains far less pack than the reference.

#### ems-ftp75c-dp (180 s, **hifi only**, EMS `dp-replay`, FRONTIER BOUND)

- **Tests:** the non-causal lower bound, and the **first DP table ever solved
  with the braking credit in its demand model**
  (`gen_dp_ems_table.py --drag scaled-air --eta-regen 0.8`). That is the point
  of the leg: a credit-free table must supply with hydrogen the SoC the run gets
  back from braking, so its total is inflated and the run's deviation against it
  is correspondingly optimistic. The `regen_bound` correction that
  `hil_report_analysis.matched_dp_for_run()` prices per run goes to zero here.
- **hifi only,** for `ems-dp-replay`'s reason exactly: the table is solved
  `--charger-accounting physical` and `bind_scenario()` refuses the mismatch at
  startup. That bind now carries a **third era guard**: the table's `drag` and
  `eta_regen` must match the run's plant. Its message states that the
  profile fingerprint cannot catch the `eta_regen` half at all and catches the
  `drag` half only when the scenario declares the key.
- **The table:** `tools/dp_tables/dp_ems_table_ems-ftp75c-dp.csv`. ⚠️ **A bare
  `--scenario ems-ftp75c-dp --force` does NOT regenerate it.** `--eta-chg` and
  `--loss-map` default to their own era sentinels, so that command solves a
  1:1-charger, loss-map-free table which `bind_scenario()` then refuses. The
  table's own `# command:` header line is the authoritative recipe and is
  reconstructed from the parsed arguments for exactly this reason:

  ```
  C:/Users/ricky/miniforge3/python.exe tools/gen_dp_ems_table.py \
      --scenario ems-ftp75c-dp --soc0 0.7 --capacity-ah 5.0 --stage-dt 0.1 \
      --lambda-dev 0.0 --lambda-term 1.0 --n-share 41 --soc-step 5e-06 \
      --charger-accounting physical --run-exit 176.0 \
      --match-terminal-soc heuristic --match-tol 2e-06 \
      --eta-chg 0.88 --loss-map plant --drag scaled-air --eta-regen 0.8 --force
  ```

  `--drag` alone would have sufficed for the road load, because the scenario
  declares the key; `--eta-regen` is then derived from it. Both are written out
  because the header line records the resolved values.
- ⚠️ **The bound is not strictly below the causal reference on this stimulus.**
  The offline matched solve reads the DP **+0.06 % above** the `soc-band` walk at
  matched terminal SoC (0.00598238 g against 0.00597881 g; target 0.697961,
  `lambda_term` 2.48383 in 21 solves, residual +1.82e-06 SoC). That is the
  discrete control grid; `LAMBDA_TERM` to terminal SoC is monotone but not
  continuous, and it is why the tuple's vs-bound arm reads about 1.01. Walk:
  h2 0.006619509 g, ΔSoC −0.001781.
- **All five legs carry a prefilled `dp_db` record.** Their 180 s duration is
  above `MATCHED_DP_LONG_DURATION_S` 100.0 s, so an on-the-fly matched solve
  would need `--matched-dp-allow-long`; prefilling moves it off the campaign
  critical path instead. Each was solved at that leg's own governor-walk
  terminal SoC, at `--eta-chg 0.88 --loss-map plant` with the road load and the
  regen era resolved from the scenario. Total 1367 s, 22.8 min.

  | Leg | target SoC | `h2_dp` (g) | walk h2 (g) | walk vs bound | converged |
  |---|---|---|---|---|---|
  | `ems-ftp75c-5050` | 0.698086 | 0.006287189 | 0.006288839 | +0.026 % | yes |
  | `ems-ftp75c-socband` | 0.698150 | 0.006444612 | 0.006448769 | +0.065 % | yes |
  | `ems-ftp75c-sdp` | 0.699536 | 0.010330269 | 0.009897751 | **−4.187 %** | **no** |
  | `ems-ftp75c-dp` | 0.698219 | 0.006616039 | 0.006619509 | +0.052 % | yes |
  | `ems-ftp75c-mpc` | 0.696885 | 0.003305625 | 0.003311646 | +0.182 % | yes |

  Four of the five sit within +0.19 % of their bound, against the +4.35 %
  the credit-free `ems-ftp75-dp` bound carried before this round. That
  agreement is the regen era working: the run and the bound now earn the same
  braking credit. ⚠️ **The `-sdp` row is the exception and it is not noise**;
  see that leg's entry for why its matched-DP figure is not a bound at all.

#### ems-ftp75c-mpc (180 s, any engine, EMS `mpc-sto`)

- **Tests:** the governor-aware receding-horizon controller, and the candidate
  of the `ftp75c-mpc` tuple. Reference and bound are the sibling tuple's, so the
  difference between the two records is the difference between `sdp-v4` and
  `mpc-sto` and nothing else.
- ⚠️ **One thing here is true of no other MPC leg:** the planner's prediction
  model carries the regen credit, so its charge enumeration runs against a mask
  that excludes every regen-capable stage: the exclusivity term, the host-side
  image of `assertFcChargeEnable()`. The credit is share-independent, so it
  cannot change which candidate the search prefers on a braking stage; what it
  can change is the terminal SoC the Huber cost is priced against. Read
  `mpc_share_pred_err` on this leg before reading its ratios; the band is 0.30,
  as on every other MPC leg.
- **Pass/fail:** the family's four regen checks appended to the standard MPC
  list over (5.0, 175.0) s. This leg is built by `_mpc_expectation()` and not by
  the family builder, so it carries `mpc_fc_peak_bounded`, the planner's own
  0.85 × `LIMIT_I_FC_MAX` budget, in place of the family's per-leg `I_fc`
  ceiling. Its walk peak `I_fc` is 0.0912 A, **the lowest of any registered
  drive-cycle leg**, so that bound has very wide slack here.
- ⚠️ **No degenerate-constant share guard, and the absence is deliberate.** The
  walk commands a constant 0.2500 for the whole cycle, range exactly 0.0000,
  because the compensated demand is low enough that the planner rails to the
  battery-heavy end of its ladder and stays there. That is the metric working:
  this leg drains the pack hardest (ΔSoC −0.003115 against the reference's
  −0.001850), burns the least hydrogen (h2 0.003311646 g) and has the lowest
  eq-H2 of the four precisely because the two move together. If a campaign shows
  the share moving, **that** is the finding, because it would mean the live plant's
  demand is materially above the walk's.

#### The two frontier tuples

`ftp75c` is a different `ems_v_profile`, a different `duration_s` and a
different `drag`, so it fails the stimulus-coherence precondition against the
`ftp75` legs and carries **its own tuples** rather than being slotted into the
existing entry. `ftp75c` takes reference `ems-ftp75c-socband`, candidate
`ems-ftp75c-sdp`, bound `ems-ftp75c-dp`; `ftp75c-mpc` substitutes
`ems-ftp75c-mpc` as candidate. Both carry the `ftp75` tuple's thresholds
verbatim, `vs_reference_max` 1.02 and `vs_bound_max` 1.06, so the compressed and
uncompressed records read on one scale. The governor walk gives `ftp75c`
vs_reference **1.0093–1.0169** and vs_bound **1.0090–1.0162**, and `ftp75c-mpc`
vs_reference **0.9862–0.9931** and vs_bound **0.9855–0.9928**, across the whole
λ band [0.409, 0.415]. All four asks are cleared. However, the `ftp75c`
vs-reference arm sits within **0.3 % of its 1.02 ask** at the top of the band, so
a first campaign reading just over 1.02 is a calibration event rather than a
policy failure.

**Campaign cost:** five legs at 180 s plus five 5 s settle pauses, **925 s or
15.4 min** on bench. `run_hil_suite.py --list` prints the figure alongside the
other opt-in sets, with an extra line recording that this set is the only one
