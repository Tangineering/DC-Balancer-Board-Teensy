...
- **Matched-DP solve done (after the digest):** (H-20 re-solve, 26 rows, residuals <= 2e-6 except sdp-braking -6.3e-4 whose target sits above the start SoC; 4-685 s each).** Run hydrogen vs its delta-SoC-matched H-20 bound: dp-replay legs +0.2 / +0.2 / +0.8 % (the plant vs the stage cost - the retired law's 0.09-0.80 % BELOW bias is gone, runs now land ABOVE); MPC legs +0.2 to +1.2 % (mpc-cross -0.2 %); hold-5050 +0.3 / +0.6 / -0.6 %; alpha-cal -0.1, greedy +0.2, charge +1.2 %; the SDP v6 / socband legs 2.4-7.5 % ABOVE their bounds (ems-sdp +6.0, sdp-braking +7.5, sdp-cross +5.4, ftp75-socband +4.5, soc-band +2.4, ftp75-sdp +1.5 %; the ftp75c family +0.1-0.5 %). The matched axis says the same as the eq-H2 axis: under the convex map the v6 SDP's FC-only clamp and the socband's charge windows are the expensive behaviours; the MPC family tracks its bound. Records in tools/dp_db (27 files, committed).
