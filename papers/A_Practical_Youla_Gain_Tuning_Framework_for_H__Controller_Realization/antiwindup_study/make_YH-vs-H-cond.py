...
fig,ax = plt.subplots(1,2,figsize=(3.45,2.1),gridspec_kw={'wspace':0.55})
st = {'H':dict(c='b',ls='--',lw=1.2,label=r'$H_\infty$, conditioned'),
      'YH':dict(c='k',ls='-',lw=1.2,label='Youla-H, conditioned')}
for name in ['H','YH']:
...
h,l=ax[1].get_legend_handles_labels(); fig.legend(h,l,frameon=False,fontsize=7,loc='lower center',ncol=2,bbox_to_anchor=(0.5,-0.01)); fig.subplots_adjust(bottom=0.34)
for a in ax: a.grid(alpha=0.25, lw=0.4)
fig.savefig(os.path.join(HERE,'figures','python','YH-vs-H-cond-18.png'),dpi=300,bbox_inches='tight'); fig.savefig(os.path.join(HERE,'figures','python','YH-vs-H-cond-18.pdf'),bbox_inches='tight')
