...
decentralization is justified rather than merely convenient, and that measuring $\Delta V_0$ on
the bench is the single highest-value action for narrowing the remaining uncertainty. A
secondary methodological lesson is that the actuator limit is a design node, not a datum: it
...
against its own uncertainty family. That separation was assumed rather than derived: the two
loops share a bus, and every ampere the motor draws is an ampere the two sources must split.
The question this report answers is therefore whether the decentralized design is genuinely
...
A study of this shape can fail in two symmetric ways: it can be set up so that the centralized
controller cannot be outperformed (by evaluating the decentralized pair on a diagonal plant),
or so that it cannot outperform (by comparing on metrics that saturate the actuator). We avoid
the first by closing \emph{both}
controllers against the same coupled $2\times2$ plant with the same assembly routine, corner sets
and simulator; we address the second by stating the amplitude of every excitation and flagging
every case in which the $\pm\SI{20}{\ampere}$ motor-current clamp is active.
...
\item A $2\times2$ small-signal design plant for the coupled share/drive problem
      ...
      static share law rather than asserted, and is reproduced endogenously to $0.032\,\%$ by an
...
\item A matrix generalization of the scalar Youla gain-tuning recipe that enforces $T(0)=I$ on a
      ...
      an exact rank-2 integrator split (Section~\ref{sec:youlah}). This is the report's
      control-theoretic contribution.
...
\paragraph{The actuator limit, and why it is set where it is.}
...
is quoted. The consequence for reading this report is that a clamped case is a case in which
the powertrain as a whole is at its limit, not one in which an arbitrarily chosen number is.
...
$b_{\!e\!f\!f}$. The gain uncertainty $K_v\in\{0.5,1,2\}$ is structural, not a tolerance: the
...
$G_{21}=0$ is a modelling \emph{result}, not an assumption. The droop ratio reaches no state that
...
--- nowhere near duty saturation anywhere in the envelope. It would break under deep battery
...
estimate; the plant is therefore assembled as explicit block state-space rather than a
...
\paragraph{Sign uncertainty --- the central limitation of the whole study.}
...
evaluation below, and either outcome is a reportable result rather than a failure.
...
$\Delta V_0 = \pm 0.4$. This is a property of \emph{triangularity}, not of weak coupling:
...
share by far more than a full-authority droop command can. That, not the RGA, is the number that
...
meaningless where it puts $\alpha_0$ outside $(0,1)$ --- and are skipped and counted rather than
...
the break was moved to $2.5\times$ the $W_p$ corner, preserving the intent. Second --- and this
is the structural finding --- the achieved crossover lands consistently \emph{below} the $W_p$
...
A $\gamma$ of \num{13.03} means the weights are not met, not that the controller is deficient: the
...
The one implementation surprise is anti-windup, and it drives the cost comparison of
...
wind up independently of the integrator. The failure is therefore \emph{amplitude-bounded}, and
the boundary is worth stating as an amplitude rather than as a verdict. A step-amplitude scan of
...
constraint on this plant turns out to be corner robustness, not nominal performance, so
...
not a tuning preference. Two features of the solution matter for
interpreting every $\gamma$ in this report. First, the measurement-equals-reference structure
...
Two structural checks follow as a byproduct. Because $G_{21}=0$, any MIMO controller restricted to
$e_1 = 0$ acts as a drive-channel SISO controller, so $\gamma_{MIMO} \ge \gamma_{drive}$
necessarily; measured, $1.6456 \ge 1.6429$, equal to $0.16\,\%$. That confirms the machinery is
consistent and shows \textbf{the coupling costs essentially nothing}: the centralized controller
...
generalizes. It is worth being explicit about what is being repaired and why. The Youla route of
...
which actuator answers which error --- a different controller, not a corrected one.
...
The correction is a small adjustment rather than a substantial repair, which is the expected
and desirable outcome. Here
...
$10^4$ (a large $\|M-I\|$ would have indicated the weights, not the realization, needed work).
...
\paragraph{Why not the direct MIMO Youla route?}
...
Table~\ref{tab:K-entries} settles the question, and its answer is the report's principal result.
...
do about a share error, and its $\SI{7.4e-4}{}$ ratio is numerical residue --- a positive check
that the synthesis discovered the plant's triangularity by itself. $K_{12}$ at $2.15\,\%$ of
...
$K_{11}$ must be large (the share loop's \SI{110}{\radian\per\second} bandwidth), not because the
term is inactive. Its magnitude is bounded by the $\Delta V_0$ sign uncertainty by construction:
a feedforward built at $+\SI{0.2}{\volt}$ is the wrong sign at $-\SI{0.4}{\volt}$, and the
synthesis correctly assigns it little authority.

\textbf{The centralized controller is about $98\,\%$ decentralized by gain.} That is the most
direct available statement that decentralized control here is justified rather than merely
convenient --- and it comes from the centralized design itself.
...
spectrum, not by a target: the singular-value tail is flat here (states 6 and 7 carry
...
and checked rather than assumed, since Nyquist at \SI{1571}{\radian\per\second} is still
...
The result is mildly counter-intuitive: \textbf{the two are comparably cheap, with the
...
$10\,\%$ margin is not an argument for either architecture, and it should not be reported as one:
the defensible statement is that \textbf{compute cost does not discriminate here}. Both figures are
...
\paragraph{The drive channel is rate-limited by the actuator, and every dynamic claim must state
the amplitude at which that limit becomes active.}
...
\subsection{Setup and what makes it fair}
\label{sec:fairness}
...
\paragraph{A declared substitution: corner family in place of a small-gain certificate.}
...
The consequence must be stated plainly: \textbf{$W_{\Delta}$ in this report is a shaping filter,
...
One further limitation must be stated explicitly: the two designs use different weight
...
quoted side by side}. The operative question --- which controller performs better on the real
coupled plant? --- is answered by the closed-loop harness regardless.
...
\paragraph{Metric 1 --- worst-corner sensitivity: a genuine centralized advantage.}
...
\paragraph{Metric 2 --- drive-transient share excursion: decided by the sign of $\Delta V_0$.}
This is the most clearly resolved result in the study. At the sign of $\Delta V_0$ the centralized
...
controller on a linear plant sees only the magnitude of the coupling, not its sign. Splitting the
...
medians is the robust statement}: the magnitude of the benefit is a modelling choice, the fact
that it reverses with an unmeasured sign is not. The saturated large step
...
\paragraph{Metric 3 --- regen bus excursion: not a discriminator.}
...
\textbf{essentially controller-independent}. The reason is worth stating precisely, because the
obvious explanation is wrong here. The event is \emph{not} rail-dominated --- the current command
...
\textbf{it is not one, and it should not be carried forward as one} --- it measures the
excitation, not the controller. On the same event the centralized controller happens to be the
better of the two on share excursion as well ($0.1448$ against $0.1747$) and recovers the bus
faster (\SI{0.43}{\second} against \SI{0.58}{\second}), which only reinforces that the bus number
itself is uninformative (Appendix~\ref{app:corners}).

\paragraph{The input node: $\bar\sigma(S_u)$.}
...
input node raises no new concern for either architecture}, and in particular the decentralized
baseline is not quietly paying for its output-sensitivity deficit at the actuator.
...
($\bar\sigma(S_o) = 4.93$ centralized against $5.77$ decentralized) --- reported rather than omitted. (iii)
...
\item \textbf{The coupling is real, one-directional, and operating-point dominated.} The droop
      ...
      the normalized plant. \textbf{The RGA
      is the wrong instrument for this plant}, which is itself a reportable finding.
\item \textbf{Decentralized control is justified, not merely convenient.} It is stable at all
      ...
      saturation; it pays a bounded price in worst-corner sensitivity ($2.55$ vs.\ $1.88$ on the
...
\item \textbf{The three primary metrics give three different answers}, and this is the result
      rather than an inconvenience: the centralized controller is superior on worst-corner
      ...
      opposing and matching signs. \textbf{It is no longer true that the decentralized pair is
...
\item \textbf{The MIMO Youla-H $T(0)=I$ correction works and is cheap.} A constant right
...
$5\,\%$. Two limitations reported above are consequences of that cross-check rather than
concessions made in spite of it: the Tier-2 worst-corner figure is a representative-subsample
...
($\SI{1.109e-2}{}$, matched exactly by both tools) rather than in continuous time.
...
(cheap, blocking for everything downstream), then $k_t$ (requires selecting a motor), then
...
fails on that spread. A sweep is a lower bound --- the conservative direction for a ``does the
truth model look worse'' check --- but not a proof.
...
constant, it is not. This is a finding about the recipe's applicability domain, not a defect in
it, and both the SISO and MIMO drive designs reach it independently.
...
Iteration~3a is the counter-intuitive one: a \emph{lower} $\gamma$ bought a \emph{more
...
the break frequency must sit \emph{below} the worst-case transport delay's phase crossover
($T_{d,v}\in[1,4]\si{\milli\second}$, i.e.\ \SIrange{250}{1000}{\radian\per\second}), not at it.
...
buys margin but puts $W_u$ and $W_p$ in direct contention over the same decade.}
\label{tab:wu-sweep}
...
A numerical note belongs with the table: the corner result is sensitive to the balanced
...
cost of two extra states (Section~\ref{sec:implementation}), which is the cheaper of the two
prices.
...
\paragraph{The $P_f$-Riccati degeneracy.}
...
\paragraph{Consequence: the Riccati $\gamma$ is optimistic in MIMO.}
...
coefficient rounding is not applied to numbers spanning two decades of physical units. One
consequence deserves emphasis, because it is easy to miss: $D_u$ carries the actuator limit into
...
not a marginal design pole. The one asymmetry worth naming is that the decentralized pair's
...
make the comparison partly an anti-windup-recovery comparison rather than a purely linear one.
The \SI{60}{\second} horizon was chosen so the slow centralized recovery is
measured rather than reported as a horizon artefact.
...
The regen event is run against the 15-state truth model with a bus-voltage output, not the
...
the new steady-state load operating point (the vehicle has stopped), not a tracking error;
...
\begin{figure}[H]
    ...
    current both controllers reach, which is a property of the demand and of the plant, not of
...
residual share error --- the authority loss itself, not a hang --- and neither shows a limit cycle
...
demanding it, not about disturbance rejection.
...
disturbance over 22--\SI{25}{\second} is the clearest decentralized advantage in the report: the
...
(\num{0.0091} against \num{0.0097}). One result of note: the centralized controller drives
...
\item \textbf{$\Delta V_0$ sign uncertainty bounds any feedforward benefit.} The entire
      ...
      claim is that the centralized design buys coupling rejection at the design sign and
      forfeits it at the mirror sign.
...
Three rows carry interpretation. The \texttt{hinfsyn} level of \num{1.0851} lands between the
...
reconfirmed here, which is the evidence that both are structural rather than artefacts of a
...
attributable to the stated approximations rather than to tool disagreement.

\paragraph{Reproduction tolerance.}
The share-channel controller is re-derived here from its documented weights rather than parsed
...
Actuator demand is not an afterthought in this formulation: for the standard configuration
$\hat u/\hat n = -Y$ (\citep{assadian2020neoclassical} \S3.8.2, eq.~(3.124), p.~132), i.e.\ the
...
measured per frequency rather than against a single global scale, which would be dominated by the
