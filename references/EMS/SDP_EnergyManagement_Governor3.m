function [P_batt_applied, P_fc_applied, SOC_out, gov] = SDP_EnergyManagement_Governor3(P_dem_array, SOC_initial, TPM, cfg)
...
% ========================================================================
% 2. VALUE ITERATION  (skipped when a matching policy cache is supplied)
% ========================================================================
% The policy depends on alpha, the grids, the H2 map and the TPM -- NOT on
% the governor. Sweeping a governor parameter therefore recomputes an
% identical J and U_star every call. Pass cfg.policy_cache (returned as
% gov.policy_cache) to reuse it; the signature below guards against reusing
% a policy computed for different settings.
c_start = cfg.h2_start_cost_g;      % g of hydrogen charged per cold start
                                    % assigned here, ABOVE the cache guard, so
                                    % it exists on the cached path too
...
policy_sig = struct( ...
    ...
    'SOC_hi',      SOC_grid(end), ...
    'n_u',         nU, ...
    'use_interp',  cfg.use_interp, ...
    'tolerance',   cfg.tolerance, ...
    'h2_a',        h2.a, ...
    'h2_shutdown', h2.shutdown, ...
    'c_start',     cfg.h2_start_cost_g, ...
    'nPd',         nPd, ...
    'TPM_sum',     sum(TPM(:)), ...
    'TPM_trace',   trace(TPM));

use_cache = ~isempty(cfg.policy_cache) && ...
            isfield(cfg.policy_cache, 'sig') && ...
            isequaln(cfg.policy_cache.sig, policy_sig);

if use_cache
    J      = cfg.policy_cache.J;
    U_star = cfg.policy_cache.U_star;
    if cfg.verbose
        fprintf('Value iteration: reused cached policy\n');
    end
else
...
end  % end of the value-iteration / policy-extraction branch
...
gov.n_latch_events = sum(diff([false (latchFC_log | latchBT_log)]) == 1);
gov.J             = J;
gov.U_star        = U_star;
gov.SOC_grid      = SOC_grid;
gov.policy_cache  = struct('sig', policy_sig, 'J', J, 'U_star', U_star);
...
gov.summary = struct( ...
    ...
    'n_latch_events',   gov.n_latch_events, ...
...
d = struct( ...
    ...
    'tolerance',       1e-3, ...
    'use_interp',      false, ...
    'policy_cache',    [], ...      % reuse J/U_star across governor sweeps;
    ...                             % see gov.policy_cache   % false = original nearest-grid snapping,
