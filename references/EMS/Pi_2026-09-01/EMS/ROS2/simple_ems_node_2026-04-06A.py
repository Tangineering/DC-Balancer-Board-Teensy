...
# Hardware limits
P_FC_MAX = 24.0   # W — H-20 peak power
...
class SimpleEMSNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(
            ...
            f"SOC_LOW={soc_low} SOC_HIGH={soc_high}"
        )
    ...
    def _cb_state(self, msg: Float64MultiArray):
        """
        /vehicle_state layout (15 elements):
        [0]=v_actual [1]=V_batt [2]=I_batt [3]=I_charge [4]=V_fc
        [5]=I_fc [6]=V_bus [7]=P_motor [8]=share_echo [9]=share_actual
        [10]=droop_FC [11]=droop_BT [12]=charger [13]=faults [14]=SOC
        """
        if len(msg.data) < 15:
            return
        self.v_actual    = msg.data[0]
        self.SOC         = msg.data[14]
        self.fault_flags = int(msg.data[13])
    ...
    def _loop(self):
        ...
        if soc < self._soc_low:
            # SOC too low — FC only, charge battery
            mode   = MODE_FC_ONLY
            share  = 1.0
            P_fc   = min(self.P_demand, P_FC_MAX)
            P_batt = self.P_demand - P_fc

        elif soc > self._soc_high:
            # SOC high enough — battery only, rest FC
            mode   = MODE_BATT
            share  = 0.0
            P_fc   = 0.0
            ...
            # Hybrid — linear ramp
            # share = 1.0 at SOC_LOW, 0.0 at SOC_HIGH
            mode  = MODE_HYBRID
            share = round(
                1.0 - (soc - self._soc_low) / (self._soc_high - self._soc_low),
                3
            )
